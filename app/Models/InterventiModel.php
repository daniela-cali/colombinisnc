...
class InterventiModel extends Model
{
    ...
    /** Classe della serie in settings; il prefisso di default vale per gli interventi manuali,
     *  le visite extra hanno il loro, e quelli da abbonamento usano
     *  tipi_intervento.prefisso_codice (PIS, ADD, ...). */
    const CLASSE_NUMERATORE = 'Interventi';
    const PREFISSO_CODICE   = 'INT';
    const PREFISSO_EXTRA    = 'EXT';
    ...
    protected function normalizza(array $data): array
    {
        ...
        if (! array_key_exists('id', $data)) {
            ...
            if (empty($data['data']['codice'])) {
                // Il prefisso dice che lavoro è: lo decide il tipo intervento (SAL, FIL, PIS...),
                // che nasca da un abbonamento o inserito a mano. INT resta per i tipi senza
                // prefisso e per gli interventi senza tipo.
                //
                // Le visite extra fanno eccezione e restano EXT: non sono una categoria di
                // lavoro ma una prestazione fuori dalle scadenze previste, da trattare a parte.
                $prefisso = self::PREFISSO_CODICE;
                if (! empty($data['data']['extra'])) {
                    $prefisso = self::PREFISSO_EXTRA;
                } elseif (! empty($data['data']['tipo_intervento_id'])) {
     ...
     *
     * Ogni prefisso ha la sua serie, creata al primo utilizzo: nessuna migrazione da scrivere
     * quando nasce un nuovo tipo intervento. Il contatore atomico e le ragioni per cui non si
     * usa MAX(codice) né AUTO_INCREMENT stanno in NumeratoriModel.
     */
