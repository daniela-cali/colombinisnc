...
/**
 * Ricrea le VIEW di CreateViewsAbbonamentiInterventi (2026-07-06) usando la colonna generata
 * clienti.denominazione al posto del CASE WHEN duplicato in ogni VIEW. Migration separata invece
 * di modificare quella originale: clienti.denominazione esiste solo da AddDenominazioneToClienti
 * (2026-08-10), successiva — modificare la migration del 6 luglio in-place avrebbe rotto un
 * 'php spark migrate' da zero, che la esegue prima che la colonna esista.
 */
class UpdateViewsAbbonamentiInterventiDenominazione extends Migration
{
    public function up(): void
    {
        $this->db->query("
            CREATE OR REPLACE VIEW v_abbonamenti_clienti AS
            SELECT
        ...
        $this->db->query("
            CREATE OR REPLACE VIEW v_abbonamenti_clienti_interventi AS
            SELECT
                a.id AS abbonamento_id,
                a.stato AS abbonamento_stato,
                CASE WHEN a.data_fine < CURDATE() AND a.stato = 'attivo'
        ...
        $this->db->query("
            CREATE OR REPLACE VIEW v_interventi_clienti AS
            SELECT
                i.id AS intervento_id,
                i.codice AS intervento_codice,
                i.stato AS intervento_stato,
