...
- [APP] **I pulsanti smettono di spostarsi da una pagina all'altra.** La regola ora è una sola: in alto gli strumenti che ti portano altrove — Modifica, Stampa PDF, Nuovo X — e in basso le azioni che decidono qualcosa e chiudono il discorso lì: Salva, Annulla, Elimina, i cambi di stato. Prima le stesse tre azioni di un form stavano in cima in tre sezioni e in fondo in altre tre, e ci si doveva ri-orientare a ogni pagina
- [APP] **Modifica ha un aspetto solo.** Aveva quattro vestiti diversi: blu pieno negli interventi, testo grigio senza colore nel personale, una matita spenta in cantieri e abbonamenti. Ora è ovunque il pulsante contornato blu con la matita e la parola, lo stesso che la scheda cliente aveva già giusto
- [APP] **Anche "Nuovo" era diverso**: quattro elenchi dicevano "Nuovo cliente", "Nuovo articolo" per esteso, cantieri e abbonamenti dicevano solo "Nuovo" e per giunta senza colore. Ora dicono tutti cosa stanno per creare, e restano pieni: su un elenco quella è *l'azione* per cui sei lì, e si distingue dagli strumenti contornati
- [APP] **Su telefono le intestazioni non si gonfiano più**: nelle schede resta la sola icona con il suggerimento al tocco, e l'etichetta ricompare da tablet in su. In compenso ogni pulsante icona ha ora un'area toccabile di almeno 44px, perché una matita piccola da centrare in cantiere è un problema vero
- [APP] **Nei form di clienti, personale e articoli Salva è sceso in fondo**, dove si finisce di scrivere, invece di stare in cima dove bisognava risalire. In `magazzino/articoli/nuovo` Annulla e Salva erano agli estremi opposti della pagina: ora sono vicini
- [APP] **Le tre pagine dell'import clienti avevano i pulsanti più grandi di tutte le altre** e "Carica e continua" non arrivava al bordo. Ora sono come il resto del gestionale
- [APP] **Righe per pagina "Tutti" ora c'è in tutte le tabelle**, non solo in tre: mancava a cantieri, abbonamenti, utenti app, clienti da migrare e alle tre tabelle dei tab della scheda cliente
- [APP] **I messaggi di errore non nominano più le colonne del database.** Al posto di «Il campo "tipo_intervento_id" è obbligatorio» ora si legge «Il campo "Tipo di intervento" è obbligatorio». Vale per tutte le validazioni del gestionale, non solo per quella segnalata
- [APP] La pagina Materiali di un cliente era bloccata a 900px e lasciava mezzo schermo vuoto: ora arriva a 1200px
- [APP] Il "Salva modifiche" di `abbonamenti/edit` era giallo, unico in tutto il gestionale: uniformato al blu degli altri
- [DEV] `.intervento-azioni` generalizzata in **`.card-azioni`** con quattro classi semantiche — `.azione-primaria`, `.azione-secondaria`, `.azione-ritorno`, `.azione-distruttiva` — che determinano ordine e posizione via `order`, così non dipendono più da come è scritto il markup. La classe va sul **figlio diretto** del footer: l'`<a>`, il `<button>`, o il `<form>` che lo racchiude quando l'azione è un POST, perché è quello l'elemento flex. Le regole desktop passano dalle utility Bootstrap nelle view al CSS
- [DEV] Il `padding-bottom` che lascia spazio al CTA ancorato su mobile resta **incondizionato**: `:has()` avrebbe evitato il vuoto nei footer che il CTA non ce l'hanno, ma richiede iOS Safari 15.4+, e su un telefono più vecchio l'ultimo pulsante finirebbe coperto e non cliccabile
- [DEV] **Nuovo `public/js/datatable-init.js`**: le dieci tabelle ripetevano le stesse quindici righe di configurazione, quindi ogni decisione comune andava applicata a mano in altrettanti file — aggiungere "Tutti" aveva richiesto di toccarne sette. Ora la view passa solo `columnDefs`, `order`, il `pageLength` quando differisce e i messaggi che nominano l'entità. Agganciato in `partials/datatables_scripts.php`, non nelle view: così nessuna può dimenticarselo e una tabella nuova lo eredita
- [DEV] La fusione dei default è a tre livelli e non un `Object.assign` liscio: `language` va fuso chiave per chiave, e dentro di esso anche `paginate` e `lengthLabels`. Altrimenti una view che sovrascrive il solo `emptyTable` cancellerebbe tutte le traduzioni e la tabella tornerebbe in inglese — senza errori, quindi difficile da notare
- [DEV] **Corretto un difetto silenzioso in `magazzino/articoli/index`**: non aveva `pageLength`, quindi mostrava le 10 righe di default di DataTables mentre la sua tendina partiva da 25 — un valore visualizzato che nel menu non esisteva. Ora prende 25 dall'init condiviso
- [DEV] **Nuovo `app/Helpers/validazione_helper.php`** e `BaseController::validate()` sovrascritto: le etichette dei campi si aggiungono in un punto solo invece che nei 35 punti in cui i controller validano, e nessun call site cambia. `etichetta_campo()` cerca in una mappa corta per i casi che una regola meccanica sbaglierebbe (`piva`, `cfisc`, `citta`, `lat`) e altrimenti ripiega su una trasformazione automatica — toglie il suffisso `_id`, underscore in spazi, iniziale maiuscola. È il ripiego la parte importante: un campo aggiunto domani non mostrerà comunque mai il nome grezzo della colonna
- [DEV] Le regole già in forma array conservano i loro `errors`: ricevono solo la `label`, quindi i messaggi personalizzati sulle date degli abbonamenti restano intatti. Le regole col jolly (`periodi.*.frequenza`) usano l'ultimo segmento dopo il punto
- [DEV] La larghezza della pagina Materiali passa da `style="max-width:900px"` nel markup a una classe `.mat-pagina` in `custom.css`, accanto alle regole `.mat-*` che quella pagina già aveva. Il resto non si tocca: `card border-0 shadow-sm` e l'intestazione col ritorno a sinistra sono identiche a `clienti/show`, e allinearle allo stile delle altre card avrebbe reso la sotto-pagina diversa dalla sua pagina madre
- [DEV] `CLAUDE.md`: corretta la sezione su `--card-accent`, che descriveva un sistema di ereditarietà del colore **mai esistito** in `custom.css` — i pulsanti "colorati dalla card" erano semplicemente pulsanti senza classe di variante. Aggiunte le convenzioni nuove su azioni, init delle DataTable ed etichette di validazione
- [DEV] `docs/spec/ui_azioni_coerenti_spec.md` aggiornato: la regola è stata **ribaltata** in corso d'opera rispetto alla prima stesura. Ragionava sull'oggetto dell'azione e concludeva che Modifica dovesse scendere nel footer, ma questo portava a spostare quattro schede per farle assomigliare a una. La §2.1 conserva il ragionamento, che è il pezzo che il codice da solo non racconta
...
- [APP] **Un abbonamento accettato per errore si può correggere.** Il nuovo pulsante **Annulla accettazione** riporta l'abbonamento in **Proposta** e cancella le visite generate: si sistemano date, frequenza o periodi, si riaccetta, e le visite si rigenerano da quelli corretti. Prima l'unica strada era modificare i periodi lasciando in giro le visite vecchie, che restavano quelle del contratto sbagliato
- [APP] L'annullamento viene **rifiutato se una visita è già stata pianificata, è in corso o è stata completata**, e il messaggio elenca quali sono con il link per aprirle. Una visita pianificata ha una data che il cliente conosce già, e una completata è storico: in quei casi non è un errore da correggere, è un contratto reale da disdire
- [APP] Nuovo pulsante **Elimina abbonamento**, disponibile solo su una **Proposta** — che per costruzione non ha visite collegate. Per un contratto già accettato la strada resta la disdetta, che lo conserva nello storico
- [APP] **La disdetta annulla ora anche le visite già pianificate in calendario.** Prima sopravvivevano alla chiusura del contratto: il tecnico si presentava a casa di un cliente che aveva disdetto. Il messaggio dice quante ne erano state pianificate, perché quelle date erano già state comunicate e serve una telefonata. Le visite arretrate e mai effettuate restano invece dov'erano, da annullare a mano nei rari casi in cui serva
- [APP] **Il rinnovo si prepara quando si vuole**, anche mentre il contratto è ancora in corso: serve per gli abbonamenti annuali che si preparano a dicembre per l'anno successivo. Non si rinnova un abbonamento **sospeso** (prima va riattivato) né uno non ancora cominciato, che porterebbe a saltare avanti di due anni. Il pulsante compare con le stesse regole nell'elenco e nella scheda, che finora offrivano il rinnovo su stati diversi
- [APP] **Anche un abbonamento sospeso ora scade**: la sospensione mette in pausa le visite, non allunga il contratto. Prima un sospeso restava sospeso a tempo indeterminato oltre la sua data di fine
- [APP] Guida della sezione Abbonamenti aggiornata: come si corregge un'accettazione sbagliata, quando l'operazione viene rifiutata e cosa fare, il rinnovo in anticipo, cosa comporta davvero la disdetta
- [DEV] Nuova migration `AbbonamentoPrecedenteRestrict`: `abbonamenti.abbonamento_precedente_id` passa da `ON DELETE SET NULL` a **`ON DELETE RESTRICT`**. È l'unica FK del progetto configurata per cedere in silenzio — eliminando un abbonamento rinnovato, il rinnovo perdeva il riferimento al precedente senza che nessuno se ne accorgesse. `elimina()` fa il controllo prima e spiega, così il vincolo del database resta l'ultima rete e non il messaggio d'errore che vede l'utente
- [DEV] `AbbonamentiController::annullaAccettazione()` è l'inverso esatto di `accetta()`, unico punto in cui gli interventi nascono: tenendo qui l'unico punto in cui muoiono vale l'invariante che regge il resto — **una proposta non ha mai interventi collegati**, quindi l'eliminazione da `proposta` non ha bisogno di controlli sui figli. Cancellazione e cambio di stato sono in transazione; le righe si leggono una volta sola e servono sia per la guardia sia per il conteggio nel messaggio
- [DEV] Nuovo `InterventiModel::eliminaPerAbbonamento()`, che cancella note e materiali prima degli interventi passando dai rispettivi model. Le loro foreign key sono `ON DELETE CASCADE` — verificato su `information_schema`, non dedotto dalle migration, dove l'ordine degli argomenti di `addForeignKey()` si legge male e aveva prodotto due commenti che dichiaravano `RESTRICT`, ora corretti. Le delete esplicite restano: è l'unico punto del progetto in cui gli interventi vengono distrutti, e deve dichiarare per intero cosa porta via
- [DEV] `InterventiModel::annullaPerAbbonamento()` prende `$inclusiPianificati` e restituisce `['totale', 'pianificati']` invece di `void`. Il parametro tiene distinti i due chiamanti: la disdetta include le visite in calendario, il rifiuto del ripristino dopo una sospensione no, perché lì si decide solo delle visite rimaste in pausa. I conteggi si leggono prima dell'`UPDATE`, dopo il quale lo stato di partenza non è più ricostruibile
- [DEV] **Due regole di dominio duplicate consolidate in un punto solo** (decisione 13 della spec). La condizione di scadenza esisteva in quattro copie — i `CASE` di `trovaConDettagli()` e `elencoConDettagli()` e i due `where` di `leggiScaduti()` — e ora vive in `STATI_SCADIBILI` più `selectStatoCalcolato()`. La condizione "questo abbonamento è rinnovabile" era scritta in due view **già divergenti tra loro**, e diventa `rinnovabile()` / `motivoNonRinnovabile()` sul model: la usano le due view per mostrare il pulsante e `rinnova()` per rifiutare la richiesta, quindi pulsante e rotta non possono più dire cose diverse. Il motivo del rifiuto arriva dallo stesso metodo che decide, per non rischiare di spiegare all'utente un rifiuto diverso da quello effettivo
- [DEV] Estratto `app/Helpers/interventi_helper.php` con `elenco_interventi_link()`, che era un metodo privato di `Impostazioni\UtentiController`: lo usano ora sia la sospensione di un account sia il rifiuto dell'annullamento. L'etichetta di ogni voce la decide il chiamante, perché cambia col contesto — il nome del cliente quando gli interventi sono di tecnici diversi, il codice quando appartengono tutti allo stesso abbonamento
- [DEV] Spec completa in `docs/spec/abbonamenti_annulla_accettazione_spec.md`, 14 decisioni con le alternative scartate
...
- [APP] **Impostazioni → Utenti** non è più un secondo form di modifica: diventa la vista d'insieme di chi ha accesso al gestionale, con username, nominativo, email, ruoli e stato. È divisa in due schede — **Personale** e **Clienti** — e quella dei clienti ha la ricerca, perché è destinata a diventare lunga
- [APP] **Un account non si elimina più: si sospende.** Sospendere chiude l'accesso e toglie la persona dai tecnici assegnabili, ma lascia intatti la sua scheda, i suoi interventi e le sue assenze; si riapre in qualsiasi momento. Eliminare avrebbe azzerato il tecnico su tutto lo storico e cancellato il suo diario delle assenze, dopo una sola conferma del browser
- [APP] L'eliminazione di un dipendente resta possibile **solo se non ha nessun intervento e nessuna assenza**, cioè in pratica per rimediare a un inserimento sbagliato; altrimenti viene rifiutata spiegando cosa lo impedisce e indicando la sospensione
- [APP] Sospendendo chi ha lavoro aperto, l'avviso **elenca gli interventi da riassegnare con il link per aprirli**, e gli stessi interventi restano nella card *Interventi in conflitto* della dashboard con l'etichetta «Account sospeso» finché non sono stati spostati: un messaggio che compare una volta non basta a ricordarsene
- [APP] Nella scheda dipendente il blocco **Account di accesso** (email, ruoli, password) è ora riservato agli amministratori. L'ufficio gestisce l'anagrafica e le assenze, e non crea più dipendenti — creare una persona significa creare le sue credenziali
- [APP] Non ci si può più sospendere o eliminare da soli, né togliersi il ruolo di amministratore: sono errori da cui si esce solo dal database
- [APP] La pagina **Profilo** funziona anche per chi non ha una scheda dipendente: in quel caso mostra solo email e password
- [APP] Sparisce dalle Impostazioni la card grigia **"Utenti Portale — in costruzione"**: era il segnaposto per gli account dei clienti, che ora hanno la loro scheda dentro Utenti. Lasciarla avrebbe promesso una seconda pagina separata che non verrà fatta
- [APP] Le guide contestuali di Utenti, Personale, Impostazioni e Dashboard sono state riscritte sul comportamento reale: descrivevano ancora la modifica degli account da Impostazioni e il «togli i gruppi per revocare l'accesso». Documentate per la prima volta anche le card **Assenze di oggi**, **Promemoria in arrivo** e **Interventi in conflitto** della dashboard, che non comparivano nella guida
- [DEV] **Chiusa un'escalation di privilegi che la review non aveva individuato**: un utente `ufficio` poteva aprire la propria scheda dipendente, spuntare "Amministratore" e ottenere `impostazioni.*`. La causa strutturale era la wildcard `personale.*` in `PERMESSI_UFFICIO`, che rendeva la matrice incapace di distinguere «gestire i dipendenti» da «nominare amministratori»: ora l'elenco è esplicito, con i nuovi permessi `personale.account` e `personale.elimina`. Un secondo difetto amplificava il primo — nel ciclo di rimozione dei gruppi mancava il filtro presente in quello di aggiunta, quindi un ufficio che salvava la scheda di un amministratore gli toglieva il ruolo
- [DEV] Le tre copie identiche della logica sull'account (in `UtentiController`, `PersonaleController`, `ProfiloController`) si riducono a `UserModel::creaAccount()` e `aggiornaAccount()`. Il filtro sui gruppi assegnabili è applicato **sia in aggiunta sia in rimozione**, quindi la regola vale automaticamente per ogni chiamante presente e futuro
- [DEV] `creaAccount()` è in **transazione** (punto 6 della review): l'email non aveva `is_unique` mentre `auth_identities` ha un vincolo `UNIQUE (type, secret)`, quindi un'email duplicata lasciava un utente senza credenziali e senza gruppi che però occupava lo username scelto
- [DEV] **La sospensione usa `ban()`/`unban()`, non il campo `active`**, che in questo progetto non impedisce l'accesso: `isActivated()` è `! shouldActivate() || active`, e senza azione di registrazione configurata il primo termine è già vero. Sarebbe stata una funzione che sembra funzionare e non fa niente, come il magic link del punto 7
- [DEV] `PersonaleModel::elencoPerGruppi()` esclude i sospesi dalle tendine di assegnazione, con il parametro `$includiId` che riammette il tecnico **già assegnato** nei form di modifica. Senza, il select ripiegava su «nessuno» e il primo salvataggio azzerava `tecnico_id` in silenzio — anche su un intervento completato, cioè proprio sullo storico che la sospensione esiste per proteggere
- [DEV] `InterventiModel::inConflitto()` unisce i due tipi di conflitto per la card della dashboard (assenza e sospensione), deduplicando chi ricade in entrambi; `contaApertiPerTecnico()` diventa `apertiPerTecnico()` e restituisce le righe, così il numero è `count()` e l'elenco diventa i link su cui riassegnare, in una query sola
- [DEV] Rimosse le due rotte `utenti-app/nuovo` e `utenti-app/store`, che puntavano a metodi inesistenti (punto 12), e le rotte di modifica ed eliminazione degli utenti; `defaultGroup` passa da `ufficio` a `cliente` (punto 16), perché il gruppo assegnato per default dev'essere il meno potente
- [DEV] `AdminSeeder` non crea più un account di servizio `admin` con password prevedibile: crea l'account reale della sviluppatrice con la sua scheda dipendente, leggendo l'identità dal `.env` (`admin.username`, `admin.email`, `admin.password`, `admin.nome`, `admin.cognome`). I gruppi restano nel codice, perché sono permessi e non identità. Se una chiave manca il seeder si ferma invece di ripiegare su un valore di scorta. È rieseguibile: riconosce l'account dallo username, allinea gruppi e scheda, e non tocca email e password
- [DEV] Chiusi i punti 5, 6, 12 e 16 di `docs/review/2026-08-16-review-progetto.md`. Vedi `docs/spec/gestione_account_spec.md`
